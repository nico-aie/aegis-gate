...
/// 2026-07-07 — `PUT /api/gates/egress`. Audit-mutated on/off for the EG-2
/// response sensitive-data OBSERVE rung (`detectors.egress_sensitive`). Body:
/// `{"enabled": true|false}`. Hot-applies by flipping the shared `AtomicBool`
/// the data plane reads before running `observe(...)` (no restart) and
/// persists `detectors.egress_sensitive.enabled` to the config doc so the
/// toggle survives restart. Observability ONLY: turning it on emits Detection
/// audit rows (visible in Live Feed / Audit) — it never feeds risk or blocks.
pub(crate) async fn handle_egress_put(
    req: hyper::Request<hyper::body::Incoming>,
    services: &aegis_control::dashboard_services::DashboardServices,
) -> Response<Full<Bytes>> {
    use http_body_util::BodyExt;
    ...
    let pre = mutation_preamble(&req, "egress-gate-put");
    let body_bytes = match req.into_body().collect().await {
        Ok(c) => c.to_bytes(),
        Err(_) => {
            return mutation_error_response(aegis_control::api::mutation::MutationError::Internal(
    ...
    let current = services.egress_observe_enabled.load(Ordering::Relaxed);
    let next = parsed.enabled;
    let before = serde_json::json!({ "enabled": current });
    let after = serde_json::json!({ "enabled": next });
    let (store, base_blob, expected) = match load_active_config_doc(services).await {
    ...
    let new_blob = match patch_egress(&base_blob, next) {
        Ok(b) => b,
        Err(e) => {
            return mutation_error_response(
                aegis_control::api::mutation::MutationError::Validation(e),
    ...
    let req_ctx = aegis_control::api::mutation::MutationRequest {
        ...
        request_id: &pre.request_id,
        client_ip: &pre.client_ip,
        resource: "/api/gates/egress",
        action: "egress_observe_set",
        reason: "operator toggled the egress sensitive-data observe rung",
    };
    let store_for_apply = store.clone();
    let blob = new_blob;
    let actor = pre.actor.clone();
    let toggle = services.egress_observe_enabled.clone();
    let outcome = services
        .mutate
        .apply_async(&req_ctx, before, after, move || async move {
            let res = store_for_apply
                .activate(expected, blob, &actor, "toggle egress observe rung")
                .await;
            if let Ok(crate::config_source::config_store::Activate::Applied { .. }) = &res {
                toggle.store(next, Ordering::Relaxed);
            }
...
/// `PUT /api/gates/ddos/mode` (2026-07-02) — flip the DDoS gate's
/// **interop mode override** between `enforce` and `log_only` from the
/// dashboard, so a `set_profile log_only` on `ddos` can be re-enforced
/// without the loopback `/__waf_control/set_profile`. Audit-mutated +
/// CSRF-gated (mirrors `handle_mode_put`); publishes the new mode map
/// fleet-wide so peers converge. This targets the interop `ModeStore`
/// only — the config-level `ddos.observe_only` flag is a separate
/// dry-run source, toggled via `PUT /api/gates/ddos`.
pub(crate) async fn handle_ddos_mode_put<B>(
    req: hyper::Request<B>,
    services: &aegis_control::dashboard_services::DashboardServices,
) -> Response<Full<Bytes>>
where
    B: hyper::body::Body<Data = Bytes>,
{
    ...
    let Some(rt) = services.interop.as_ref() else {
        // No interop runtime (test bundle) → the mode store the data
        // plane reads isn't wired, so there's nothing to toggle.
        return json_response(
            503,
            &serde_json::json!({
                "ok": false,
    ...
    let outcome = services
        .mutate
        .apply::<_, aegis_control::interop::headers::Mode, aegis_control::api::mutation::MutationError>(
            &req_ctx,
            before,
    ...
    match outcome {
        Ok(o) => {
            // Converge the new mode map across the fleet (mirrors the A0
...
/// Patch `detectors.egress_sensitive.enabled` for the `PUT /api/gates/egress`
/// observe toggle. Only the on/off flag is written; the detector's cap /
/// sampling / density knobs stay at their code defaults. Observability only —
/// this rung emits audit rows, never risk/block.
fn patch_egress(base: &str, enabled: bool) -> Result<String, String> {
    let mut root = yaml_root(base)?;
    let detectors = yaml_submap(&mut root, "detectors")?;
    let egress = yaml_submap(detectors, "egress_sensitive")?;
    egress.insert(yaml_str("enabled"), yaml_bool(enabled));
    serde_yaml::to_string(&serde_yaml::Value::Mapping(root))
        .map_err(|e| format!("re-serialize config: {e}"))
}
...
mod gate_toggle_patch_tests {
    ...
    #[test]
    fn patch_egress_persists_observe_toggle() {
        // PUT /api/gates/egress must persist into detectors.egress_sensitive
        // so the observe rung survives restart and reads back through
        // cfg.detectors.egress_sensitive.enabled.
        let on = patch_egress(BASE, true).unwrap();
        let parsed: serde_yaml::Value = serde_yaml::from_str(&on).unwrap();
        assert_eq!(
            parsed["detectors"]["egress_sensitive"]["enabled"].as_bool(),
            Some(true)
        );
        assert_loads(&on);
        assert!(
            aegis_core::load_config_str(&on)
                .unwrap()
                .detectors
                .egress_sensitive
                .enabled
        ...
        let off = patch_egress(&on, false).unwrap();
        assert!(
            !aegis_core::load_config_str(&off)
                .unwrap()
                .detectors
                .egress_sensitive
                .enabled
