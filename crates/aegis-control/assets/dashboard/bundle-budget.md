...
| 2026-06-23 | 640 KB | 840 KB | queue_wait latency-panel row + load-shedder wiring, the per-IP rate-limit enable/disable toggle, and the whitelist full-trust exemption UI. Measured `app.js` 597.7 KB / raw 783.5 KB; feature surface only (React UMD bundles unchanged at 10.8 + 131 KB — no new deps). app.js was at 99.6 % of the old cap, so both budgets bumped to restore ~7 % headroom. |
