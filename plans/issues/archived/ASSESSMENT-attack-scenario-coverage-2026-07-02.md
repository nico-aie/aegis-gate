...
**Derived work:** the improvement backlog has been split into two shippable plans — [`FEAT-attack-coverage-wiring-2026-07-03.md`](./archived/FEAT-attack-coverage-wiring-2026-07-03.md) (security coverage) and [`PLAN-ltester-perf-and-hardening-2026-07-03.md`](./archived/PLAN-ltester-perf-and-hardening-2026-07-03.md) (performance + hardening, from the l-tester audit). This file stays as the umbrella analysis. See *Archival note* at the bottom.
...
- ✅ **DONE — archived 2026-07-05** — [`PLAN-ltester-perf-and-hardening-2026-07-03.md`](./archived/PLAN-ltester-perf-and-hardening-2026-07-03.md): committed set merged (LT-P1 release profile, LT-P8 hygiene; LT-P2/LT-P4/LT-P1b rejected-as-unsafe, LT-P3/5/6/7 owner-skipped), moved to `archived/`. Also closed the two doc-hygiene nits from *Corrections* above (`data_plane.rs:678` now `// Enforce — 403`; `canary.rs:6` now `(score 100)`).
- **This ASSESSMENT stays live in `plans/issues/`** (2026-07-05). Both derived plans are now archived, but the second archival condition is **not** met: residual gaps this file documents are neither shipped nor ticketed as standalone issues — notably **GeoIP ASN + geo-rules dead even with GeoIP on** (`Asn`/`Country` rule conditions get an empty `EvalContext`; `kind: asn` hard-coded false; Tor/VPN classifier unwired) and **limit-change→withdrawal completely undetected** (no EndpointTag). The detection-accuracy gaps are now partly tracked by the round-3 FP + recon plans, but the GeoIP/business-logic gaps are not. Keep this as the source of truth until those are shipped or spun into their own issues.
