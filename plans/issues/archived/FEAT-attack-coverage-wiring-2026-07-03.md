...
> **Type:** FEAT (security-coverage track) · **Status:** ✅ **COMPLETE 2026-07-04 — archived.** Every item is terminal: shipped (AC-P1-a/b/c/d #127–#130 · AC-P2-e #136 · AC-P2-c #138 · AC-P2-a #139 · AC-P3-b #140 · AC-P2-d #141+#147 · AC-P3-d doc #142 · AC-P2-b #148), declined (AC-P3-e lockout — DoS-weaponizable; AC-P3-c — owner rejected per-request account parsing), or blocked-deferred (AC-P3-a real JA4 — rustls hides the ClientHello extension list; needs a raw-parse spike if ever revived). · **Branch:** `feat/attack-coverage-*` (per stage)
...
### AC-P1-a · Wire response header stripping for proxied traffic · **S** · *(vector 08 — version-banner leak)* — ✅ **SHIPPED 2026-07-03 (PR #127)**
...
### AC-P1-b · Extend the transaction shape table + add a `LimitChange` tag · **S** · *(vector 06 — missing fraud shapes)* — ✅ **SHIPPED 2026-07-03 (PR #128)**
...
### AC-P1-c · Canonicalize usernames in the brute-force per-user axis · **S** · *(vector 02 — spray evasion)* — ✅ **SHIPPED 2026-07-03 (PR #129)**
...
### AC-P1-d · XSS body decoding parity with SQLi · **M** · *(vector 07 — JSON-body XSS evasion)* — ✅ **SHIPPED 2026-07-03 (PR #130)** — perf gate held: mixed-traffic parity, 100%-large-body worst case +3.6% p99 (under the 5% ceiling)
...
### AC-P2-a · Wire `BehavioralAnalyzer` behind a default-OFF toggle · **M** · *(vectors 2, 5)* — ✅ **SHIPPED 2026-07-03 (PR #139)**
> Wired as `ProxyContext.behavior_analyzer` behind default-OFF `detectors.behavior_analyzer` (`Some` only when enabled → zero cost off). The error-ratio half landed with AC-P3-b (PR #140): `observe_outcome` on the Allow-gated response hook feeds upstream 4xx → `behavior_high_errors`. Nothing left in this stanza.
...
### AC-P2-b · Fleet-aggregate the brute-force axes — **cluster mode only** · **M** · *(vector 2, theme 2)* — ✅ **SHIPPED 2026-07-04 (PR #148)**
> `brute_force.count_scope: per_node | fleet` (default `per_node`). Fleet: each locally-NEW distinct IP fire-and-forgets an `incrby` on `bf:fleet:{axis}:{key}:{bucket}` (TTL 2×window) and reads current+prior bucket into a bounded 4096-entry cache the next `inspect` consults — `inspect()` never awaits; backend error → local count governs. Boot gate = `cluster_enabled()` AND `state.backend: redis`, else warn-once + per-node (spike_scope gap closed as specified). Chain slot holds a shared `Arc<BruteForceDetector>`; `apply_cfg_change_to_brute_force` hot-applies scope (structural guard test wired). Known bounded overcount: one IP on two nodes contributes twice. All tests below shipped as written.
...
### AC-P2-c · Fix rules-engine `Asn`/`Country` eval + wire ASN blacklist · **M** · *(vector 3, theme 3; needs GeoIP)* — ✅ **SHIPPED 2026-07-03 (PR #138)**
...
### AC-P2-d · Endpoint-enumeration / 404-rate detector · **M** · *(vector 8)* — ✅ **SHIPPED in two PRs: #141 (base, 2026-07-03) + #147 (true 404-rate, 2026-07-04)**
> #141 shipped the bounded distinct-path detector (chain-resident, default-OFF). #147 upgraded it to the true 404-rate: moved OFF the chain onto `ProxyContext.enumeration` (chain detectors can't see upstream status; mirrors `behavior_analyzer`), gate now `distinct_paths > threshold AND origin_404s > threshold` — `observe_path` inbound + `observe_outcome(status == 404)` on the AC-P3-b Allow-gated hook. Kills the 200-heavy-crawler FP the stanza worried about; still default-OFF pending FP-corpus tuning. `clear()` genuinely wired into `reset_state` (was doc-only). Cardinality bounds unchanged.
...
### AC-P2-e · Referer origin validation (opt-in, CRITICAL routes) · **S/M** · *(vector 5)* — ✅ **SHIPPED 2026-07-03 (PR #136)** — `behavior_cross_origin_referer` (score 20) in `behavior_signals`, opt-in via `detectors.referer_origin`
...
- **AC-P3-a · Real JA4 from the ClientHello.** ~~Read cipher/extension lists in the TLS layer and call the existing `ja4.rs::compute`.~~ **BLOCKED on the rustls public API (2026-07-04) — deferred, needs a spike.** `ja4::compute` requires the ClientHello's **extension list** (sorted, part of the canonical JA4 hash), but rustls does not expose it: the TLS path uses `tokio_rustls::TlsAcceptor` + a `ResolvesServerCert` whose `ClientHello` surfaces only `server_name()` / `alpn()` / `cipher_suites()` — **no extension access** — and even the low-level `rustls::server::Acceptor` (`LazyConfigAcceptor`) hands back the same limited `ClientHello`. So real JA4 requires **parsing the raw ClientHello bytes ourselves** (peek the TCP stream before rustls, decode the TLS record + ClientHello, extract cipher/extension/named-group/sig-alg lists) — a genuine TLS-parser implementation plus a per-handshake decode cost. That's its own spike, not a wiring task. The shipped **"JA4-light"** stub (`t{ver}{sni}_{cipher_hex}_{alpn_hash}`) still differentiates common clients (curl vs Chrome vs sqlmap have distinct cipher/ALPN choices) and drives the device→IP-rotation axis today; it's coarser than canonical JA4 but not blocking any *shipped* feature. Revisit only if a device-fingerprint gap is traced to the coarse fp AND the raw-parse spike is scoped. (The known-bad-JA4 blocklist `bots.rs` and TLS-fp↔UA consistency check depend on this, so they're deferred with it.)
- **AC-P3-b · Response-outcome signal channel.** ✅ **SHIPPED 2026-07-03 (PR #140).** Not via `RequestView` — landed as an Allow-gated outcome hook in the single-exit `handle_data_request` wrapper (`data_plane.rs:~135`): upstream client-error (4xx) outcomes of FORWARDED requests feed `behavior_analyzer.observe_outcome` (→ `behavior_high_errors`, closing AC-P2-a's error-ratio half) and, since PR #147, `enumeration.observe_outcome(status == 404)` (AC-P2-d's true 404-rate). WAF-origin blocks never count (the `Action::Allow` gate). AC-P3-e, the third would-be consumer, was declined below.
- **AC-P3-c · One-device-many-accounts detector.** ❌ **DECLINED BY OWNER 2026-07-04** — "don't parse an account signal on every request": the detector would need per-request username extraction on non-auth routes, hot-path cost the owner explicitly rejected. The shipped coverage already bounds the shape: `DeviceIpTracker` catches device→IP rotation, the brute-force per-device axis (now fleet-aggregating, PR #148) catches one-device-many-IP stuffing on auth paths. Won't-do; revisit only with a new owner mandate.
...
## ✅ Closed 2026-07-04 — final ledger

| Item | Outcome | PR(s) |
|------|---------|-------|
| AC-P1-a header strip | shipped | #127 |
| AC-P1-b LimitChange shapes | shipped | #128 |
| AC-P1-c username canonicalization | shipped | #129 |
| AC-P1-d XSS decode parity | shipped (+3.6% p99 worst case, under gate) | #130 |
| AC-P2-a BehavioralAnalyzer wiring | shipped (error-ratio half via AC-P3-b) | #139 (+#140) |
| AC-P2-b fleet brute-force | shipped (cluster-gated, fail-safe) | #148 |
| AC-P2-c geo rules + ASN blacklist | shipped | #138 |
| AC-P2-d enumeration / 404-rate | shipped (base + true-404 refinement) | #141, #147 |
| AC-P2-e Referer origin | shipped | #136 |
| AC-P3-a real JA4 | **blocked-deferred** (rustls API; raw-ClientHello spike required) | — |
| AC-P3-b response-outcome channel | shipped | #140 |
| AC-P3-c one-device-many-accounts | **declined by owner** (per-request account parsing rejected) | — |
| AC-P3-d L4 posture doc | shipped | #142 |
| AC-P3-e account lockout | **declined** (L7-WAF anti-pattern, DoS-weaponizable; built, tested, discarded) | — |

Residual (tracked elsewhere, NOT this plan): enumeration + behavior_analyzer remain **default-OFF pending FP-corpus tuning** before any prod enable; the l-tester perf plan (`PLAN-ltester-perf-and-hardening-2026-07-03.md`) still has open items and stays live.
