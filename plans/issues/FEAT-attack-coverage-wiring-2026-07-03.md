...
- **AC-P3-d · Decide L4 posture explicitly (doc, not code).** State that TCP/UDP volumetric floods are handled upstream (kernel/LB/anycast); confirm the app-layer `conn_limit` + accept-loop backpressure story suffices for "DDoS aimed at the WAF." Runbook + stated boundary. *Pairs with the l-tester perf plan's DDoS default-limit item.*
- **AC-P3-e · Per-account lockout primitive.** Hard lockout (not just additive score) after repeated failures on one account — **requires AC-P3-b** (failure signal) first.
...
4. **AC-P3-b** is the unlock for the outcome-blind theme — schedule it before AC-P3-e / the error-ratio half of AC-P2-a. The owner has flagged outcome-blindness as "OK for now," so AC-P3-b/-e are explicitly deferrable.
