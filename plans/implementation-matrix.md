...
**As of:** 2026-06-19 (recent ships on `develop`: **per-route monitor mode** — `RouteConfig.mode: enforce|log_only`, forwards + downgrades WAF blocks to log-only per route, blacklist stays hard [`crates/aegis-core/src/config.rs` `RouteMode`, `crates/aegis-proxy/src/data_plane.rs` `effective_mode`]; **recon_path** coverage gaps closed [`crates/aegis-security/src/detectors/recon.rs`]; **detector-breakdown** now counts detected-but-allowed hits, not just blocks [`crates/aegis-control/src/api/attacks.rs` `by_detector`]; LT-RUN-11 Redis/secrets/DoS hardening. Open backlog → [`issues/README.md`](./issues/README.md). For full state, read **git history** — the per-doc rows below are reviewed less often, so verify against the named module before relying on a row.) Earlier (2026-06-03): AI Operator Copilot P0–P4 SHIPPED + live-verified 2026-06-02, then **config centralized into YAML** 2026-06-03 — `observability.copilot` + `${secret:...}` key ref + config-plane hot-reload fold; Observability OTel→SigNoz traces smoke-verified + SLO-message-quality P1 shipped 2026-06-02; cluster config-plane + multi-node metrics track CLOSED 2026-05-27; Tier A hygiene sweep closed. **No active build track.** **Deprioritized:** API-security guard wire-up, pending a WAF-vs-gateway concept call). Earlier baseline (2026-05-19): v2.5 interop contract — challenge wire shape, `/challenge/verify` public mount, loopback-gated `/__waf_control/*`, composite-key risk + IpRateLimiter data-plane wire-up; HACK-T1..T5 + rollback v6 + MTLS-T7..T11 + TCP-T1..T6 + FDP-T1..T6 + AI-T + geoip XFF.
...
| [`smart-caching.md`](../docs/data-plane/smart-caching.md) | **Implemented (Phases 1–3)** | Per-upstream L1 in-process cache (`aegis-proxy/src/cache/`): `X-WAF-Cache` HIT/MISS/BYPASS, hard never-cache-CRITICAL, GET/HEAD allow-list, deception/poisoning-safe keys (Accept-Encoding + Vary), byte-budgeted moka eviction, `/api/cache/stats` + dashboard card, Redis pub/sub fleet-wide purge fan-out, and shared **L2 (Redis) tier** behind L1 (`--features redis`). Verified incl. live-Redis L2 integration test, and the **Redis Cluster** L2 client (`cache.l2.cluster: true` → async cluster client + per-master SCAN purge). Remaining: Phase 4 (`stale-if-error`, ETag revalidation). Spec: [`plans/future/smart-caching.md`](future/smart-caching.md); QC: [`docs/data-plane/smart-caching-qc.md`](../docs/data-plane/smart-caching-qc.md). The historical `docs/data-plane/smart-caching.md` describes the removed per-tier `TierCache`. |
...
| [`ai-operator-copilot.md`](../docs/control-plane/ai-operator-copilot.md) | **Implemented (P0–P4)** behind `--features llm` | LLM operator copilot — situational summaries + smart-catch triage over the WAF's own telemetry. Advisory-only, off the hot path, PII-redacted before egress, off by default. **Shipped (2026-06-02):** `aegis-control/src/copilot/` — `LlmProvider` trait + `CostGuard` + `redact_for_egress` (dlp) gate + `summarize()` orchestrator; **OpenAI-compatible** adapter (default, `LLM_*` env, vLLM/Ollama/OpenAI) + Anthropic adapter, behind the `llm` feature; 13 unit tests. Snapshot adapter (risk.top + by_detector + SLO labels) + `GET /api/copilot/summary` async admin endpoint shipped. **Live-verified end-to-end** against a vLLM endpoint (Qwen3.6-35B) — logged in, drove traffic, endpoint returned an accurate brief. **P2 dashboard panel shipped** (Security Ops → Copilot: on-demand brief + snapshot-beside-prose + free-form "ask" box; `GET /api/copilot/summary` + `/ask`, both live-verified). **P3 smart-catch triage** — `GET /api/copilot/suggestions` + panel card (campaign clustering + candidate rules, advisory; live-verified). **P4 scheduled briefings** — `AlertEvent::OperatorBriefing` pushed into the alerts pipeline on `LLM_BRIEFING_INTERVAL_SECS` (live-verified). **Track P0–P4 complete** + per-event clustering (audit-ring events grouped by detector → IPs + paths, fed into triage; live-verified). **Config centralized into YAML (2026-06-03):** `observability.copilot` (`CopilotConfig` in `aegis-core`: enabled/provider/base_url/model/timeout_ms/briefing_interval_secs/api_key_ref) replaces raw `LLM_*` env as the primary surface; the API key is a `${secret:...}` ref resolved per-node (`run::resolve_copilot_api_key`), never inline. `CopilotService` is now an `ArcSwap` global built via `from_config` at boot and **hot-reloaded by the config plane** (`apply_cfg_change_to_copilot` in the folded apply path — enable/disable/model/key-rotation take effect cluster-wide, no restart). Legacy `LLM_*` env is a back-compat fallback. Plan: [`plans/archive/ai-operator-copilot.md`](./archive/ai-operator-copilot.md). Distinct from the inline ONNX detector + the Tier 2 customer-LLM firewall. |

## 6. Observability

| Doc | Status | Notes / module path |
|---|---|---|
| [`prometheus-otel.md`](../docs/observability/prometheus-otel.md) | **Partial** | Prometheus scrape (`metrics/{exporter,request_duration,mod}.rs`, per-stage WAF latency F-T10) + stdout-JSON tracing live. **OTLP trace export is real** (`aegis-bin/src/otel.rs`): the 4 `opentelemetry*` deps (0.24/0.17) ARE in `Cargo.toml` + lockfile and `cargo build -p aegis-bin --features otel` ships spans over OTLP gRPC to `cfg.observability.otel.endpoint`. Chosen backend **SigNoz** (`deploy/signoz/`). Trace export **smoke-verified 2026-06-02** (spans land in an OTLP backend with `service.name=aegis-gate` + `waf.handle_data_request`/`waf.forward_upstream`). **Metrics + logs** reach SigNoz via the OTel Collector (`deploy/otel/collector.yaml`): `prometheus` receiver scrapes `/metrics`, `filelog` tails the WAF's JSON log — both → OTLP → SigNoz, no app change. Span-noise filter shipped + verified (OTLP layer `Targets` filter → only `waf.*` spans export, h2 internals dropped). **Partial** pending: metrics/logs live smoke, (optional) app-side OTLP push, SIGTERM batch flush. Plan: [`observability-otel-and-alerts.md`](./archive/observability-otel-and-alerts.md). |
...
| [`slo-sli-alerting.md`](../docs/observability/slo-sli-alerting.md) | **Implemented** | `slo.rs` — 5 SLI kinds, multi-burn windows, 5 receiver kinds. 2026-05-20 alerts refactor added `AlertEvent` router + `AlertDedupCache` + per-severity `AlertReceiver.severities` routing + `dispatch_event`; producers for the 9 non-SLO event classes + dashboard severity UI remain (see [`plans/archive/alerts-refactor.md`](archive/alerts-refactor.md)). **SLO message quality P1 shipped 2026-06-02** (`slo/dispatch.rs::format_alert_text`): severity glyph + deployment identity (`service·node·env`), clamped budget (no more `97727%`), measured-vs-target (`SloAlert.measured`/`target`), human local timestamps + relative `(N ago)`. P2–P4 (other `AlertEvent` variants, dashboard/trace deep-links, likely-cause): [`observability-otel-and-alerts.md`](./archive/observability-otel-and-alerts.md). |
...
| [`plans/archive/observability-otel-and-alerts.md`](./archive/observability-otel-and-alerts.md) | **Partial (P1 shipped)** | Observability improvements. **(1) OTel export — traces live + smoke-verified 2026-06-02** (real OTLP gRPC exporter in `aegis-bin/src/otel.rs` under `--features otel`; SigNoz backend `deploy/signoz/`; span-noise `Targets` filter). Metrics + logs reach SigNoz via the Collector (`deploy/otel/collector.yaml`); pending: metrics/logs live smoke, optional app-side OTLP push, SIGTERM batch flush. **(2) Alert messages — P1 shipped 2026-06-02** (`slo/dispatch.rs`: severity glyph + deployment identity + clamped budget + measured-vs-target + humanized timestamps). Pending: other `AlertEvent` variants, dashboard/trace deep-links, likely-cause (P2–P4). |
