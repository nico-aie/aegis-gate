...
| [`cluster-config-distribution.md`](../docs/operations/cluster-config-distribution.md) | **Partial** | Phase 0 + A-core (2026-05-27): `StateBackend::{incrby,expire,scan_prefix,cas_set}` + `config_source/config_store.rs` (`ConfigStore` — versioned `config:waf:doc`, CAS activation, immutable snapshots, rollback, per-node ACK) + `config_source/redis_source.rs` watcher (apply via `reload::` helpers, ACK/NACK/fail-static). **Not yet wired:** `PUT /api/config`, boot-site `spawn_watcher` in `run.rs`, console drift view. Plan: [`plans/future/cluster-config-sync-and-scaling.md`](./future/cluster-config-sync-and-scaling.md). |
