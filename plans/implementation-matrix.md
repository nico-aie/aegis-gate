...
| [`detectors/template-injection.md`](../docs/security/detectors/template-injection.md) | **Implemented** | `detectors/template_injection.rs` (Run-5 GAP-006 + Run-6 GAP-006b). Regex-only — Jinja2 `{{config}}` / Twig `{{7*'7'}}` (quote-tolerant arithmetic POC) / Mako / Freemarker `<#assign>` / Velocity `#set()` / SpEL `${T(...)}` / Handlebars `{{#with}}`. Score 50. |
