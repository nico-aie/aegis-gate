...
| [`smart-caching.md`](../docs/data-plane/smart-caching.md) | **Implemented (Phases 1–3)** | Per-upstream L1 in-process cache (`aegis-proxy/src/cache/`): `X-WAF-Cache` HIT/MISS/BYPASS, hard never-cache-CRITICAL, GET/HEAD allow-list, deception/poisoning-safe keys (Accept-Encoding + Vary), byte-budgeted moka eviction, `/api/cache/stats` + dashboard card, Redis pub/sub fleet-wide purge fan-out, and shared **L2 (Redis) tier** behind L1 (`--features redis`). Verified incl. live-Redis L2 integration test. Remaining: Redis Cluster client + Phase 4 (`stale-if-error`, ETag). Spec: [`plans/future/smart-caching.md`](future/smart-caching.md); QC: [`docs/data-plane/smart-caching-qc.md`](../docs/data-plane/smart-caching-qc.md). The historical `docs/data-plane/smart-caching.md` describes the removed per-tier `TierCache`. |
