...
| [`ai-operator-copilot.md`](../docs/control-plane/ai-operator-copilot.md) | **Partial (Phase 0)** | LLM operator copilot — situational summaries + smart-catch triage over the WAF's own telemetry. Advisory-only, off the hot path, PII-redacted before egress, off by default. **Shipped (2026-06-02):** `aegis-control/src/copilot/` — `LlmProvider` trait + `CostGuard` + `redact_for_egress` (dlp) gate + `summarize()` orchestrator; **OpenAI-compatible** adapter (default, `LLM_*` env, vLLM/Ollama/OpenAI) + Anthropic adapter, behind the `llm` feature; 13 unit tests. Snapshot adapter (risk.top + by_detector + SLO labels) + `GET /api/copilot/summary` async admin endpoint shipped. **Live-verified end-to-end** against a vLLM endpoint (Qwen3.6-35B) — logged in, drove traffic, endpoint returned an accurate brief. **P2 dashboard summary panel shipped** (Security Ops → Copilot: on-demand brief + snapshot-beside-prose; bundle embedded). **Remaining:** copilot "ask" box + endpoint, smart-catch triage (P3). Plan: [`plans/future/ai-operator-copilot.md`](./future/ai-operator-copilot.md). Distinct from the inline ONNX detector + the Tier 2 customer-LLM firewall. |
