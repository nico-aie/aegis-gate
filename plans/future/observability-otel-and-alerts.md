...
| **P4** | Redaction processor + tail-sampling + a documented backend matrix; `shutdown_tracer_provider` flush on SIGTERM (the `OTEL_PROVIDER` OnceLock is already parked for this). **Also: filter span noise** — the 2026-06-02 smoke showed exported traces are mostly `h2`/dependency-internal spans (`reserve_capacity`, `send_data`, `hpack::*`…) drowning the useful `waf.*` spans. Add a per-target filter (drop the `h2`/hyper internal targets) or an EnvFilter so only `aegis_*` + request spans export. | M |
