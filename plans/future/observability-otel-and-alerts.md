...
| **P4** | ✅ **Span-noise filter DONE + smoke-verified 2026-06-02.** A per-layer `Targets` filter on the OTLP layer (`otel.rs`) keeps only `aegis_*`/`waf` targets (default OFF) — re-ran the Jaeger smoke and the export dropped from `h2`/hpack internals to exactly `waf.handle_data_request` + `waf.forward_upstream`. *Remaining P4:* `shutdown_tracer_provider` SIGTERM flush (the `OTEL_PROVIDER` OnceLock is parked for it), tail-sampling, documented backend matrix. | M |
