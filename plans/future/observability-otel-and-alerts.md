...
| Phase | Scope | Est. |
|---|---|---|
| **P1** | ✅ **DONE + SMOKE-VERIFIED 2026-06-02.** OTLP **trace** export is real (deps already in `Cargo.toml`; `--features otel` compiles the exporter — corrected the stale "stub" docs). **Live smoke passed**: built `--features "redis otel"`, ran the WAF with `otel.endpoint=:4317`, drove traffic, and confirmed in an OTLP backend (Jaeger — same protocol as SigNoz) `service.name=aegis-gate` + the WAF's own spans `waf.handle_data_request` (server; tags method/path/tier/action/peer/timing) and `waf.forward_upstream` (client). SigNoz wiring: `deploy/signoz/README.md`, `deploy/otel/collector.yaml`, `observability.otel` in `dev.yaml` + `prod-strict.yaml`. **Finding:** exported spans are dominated by `h2` library-internal noise — see P4. | S–M |
| **P2** | ✅ **Collector path wired + scrape smoke-verified 2026-06-02.** `deploy/otel/collector.yaml` `prometheus` receiver scrapes the WAF's `/metrics` (admin `:9443`, matched labels) → OTLP → SigNoz. **Smoke:** ran a Prometheus scraper (the Collector's `prometheus` receiver IS the same scrape engine) against the live WAF — `up{job=aegis-waf}=1`, `aegis_*` series queryable with the `service`/`plane` labels. The scrape→OTLP→SigNoz forwarding hop couldn't be exercised here (the `otelcol-contrib` image is unpullable in this offline env) — standard Collector behaviour, not WAF-specific. *Optional* app-side OTLP metric push deferred. | S |
...
| **P4** | ✅ **Span-noise filter DONE + smoke-verified 2026-06-02.** A per-layer `Targets` filter on the OTLP layer (`otel.rs`) keeps only `aegis_*`/`waf` targets (default OFF) — re-ran the Jaeger smoke and the export dropped from `h2`/hpack internals to exactly `waf.handle_data_request` + `waf.forward_upstream`. **SIGTERM batch flush DONE + smoke-verified 2026-06-02** — `otel::shutdown()` (`force_flush` on the parked `OTEL_PROVIDER`) is called in `main.rs` after the server loop returns, inside the live runtime. Smoke: ran with `AEGIS_DRAIN_GRACE_MS=200` (≪ the 5s batch scheduled-delay), sent a request, `kill -TERM` — the `waf.handle_data_request` spans still reached Jaeger, which only the shutdown flush can do in that window. *Remaining P4:* tail-sampling + documented backend matrix. | M |
