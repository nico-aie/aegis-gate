...
| Phase | Scope | Effort |
|---|---|---|
| **P0 — Durability foundation** (prerequisite, no behavior change) | **Infra half ✅ shipped 2026-06-23 (PREREQ-A):** `redis-data` / `aegis-redis-data` volume mounted on `/data` in both shipped compose files (`deploy/compose/docker-compose.yml`, `deploy/docker-compose.dev.yml`), RDB/AOF validated via `docker compose config`, and the persistence-enabled-managed-Redis expectation documented in `deploy/helm/aegis-gate/values.yaml`. **Remaining (with P1+):** establish the `control:waf:*` interim keyspace + the reset-wiring contract in §4; serialization helpers (`serde_json`) for the three value shapes. | **S** |
