...
| **P2** | 🟡 **summary panel DONE 2026-06-02.** Dashboard `PageCopilot` (Security Ops → Copilot): "Generate brief" (on-demand, never auto-polls) + window selector; renders the brief prose beside the snapshot numbers; LLM text rendered as plain text (no HTML injection); 503/error/empty states. Bundle rebuilt (under budget), embedded. **Remaining:** the "ask" box (needs a `POST /api/copilot/ask` endpoint). | M |
