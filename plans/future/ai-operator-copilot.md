...
| **P1** | 🟡 **core DONE + live-verified 2026-06-02.** `summary.rs`: `TelemetrySnapshot` + `render_prompt` + `summarize()` (render → redact → budget-admit → provider → record-usage → `Brief`). 13 copilot unit tests. **Live smoke** against an OpenAI-compatible vLLM endpoint (Qwen3.6-35B) produced an accurate brief from a synthetic snapshot (in=286/out=162 tokens). **Remaining:** the snapshot adapter (`RiskTracker::top` + `SloEngine::active_alerts` + counters → `TelemetrySnapshot`) and the `GET /api/copilot/summary` admin endpoint. | M |
