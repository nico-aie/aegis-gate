...
| ◑ | **ddos-cross-node-rps-aggregation** — **P1+P2 shipped** (testable `tick_with_current` seam; fleet RPS aggregation via `ddos:fleet:rps:<sec>` buckets behind `ddos.spike_scope: per_node\|fleet`, default off, fail-safe to per-node; `spike_scope` round-trips the gates API). P3 dashboard JSX + P4 docs remain | Foundation | M | Cluster-wide spike signal; closes the Wave 2 availability theme |
