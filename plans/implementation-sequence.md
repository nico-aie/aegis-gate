...
| ◑ | **zone-aware-load-balancing** — **P1+P2 shipped** (P1 node self-zone `node.zone`/`AEGIS_ZONE`; **P2** `pick_with_locality` prefers same-zone members behind `pool.locality.enabled`, default off, v1 presence-gate spillover, composes with fail-open). P3 observability (dashboard readout + per-zone summary + served-local metric) + P4 capacity gate remain | Foundation | M | Locality routing; shares the `LbStrategy::pick` touchpoint |
