...
| ◑ | **zone-aware-load-balancing** — **P1 shipped** (node self-zone identity: `node.zone` + `AEGIS_ZONE` env → `ProxyContext.self_zone`, read-only plumbing, no routing change). P2 zone preference in `pick` (presence-gate spillover, composes with fail-open) + P3 observability + P4 capacity gate remain | Foundation | M | Locality routing; shares the `LbStrategy::pick` touchpoint |
