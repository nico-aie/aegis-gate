...
| ☐ | **admin-accounts-rbac-sso P1** (self-service hardening) — staged AA-P1a→d, work-item `plans/issues/FEAT-admin-accounts-p1-self-service-hardening.md`: P1a wire dormant password/session-revoke/break-glass endpoints (start, S) → P1b TOTP enroll + recovery-code login (M) → P1c fleet-wide rate-limit (M) → P1d kill committed defaults + setup token (M) → [[admin-accounts-rbac-sso]] | Foundation | S/M | Standalone enterprise-readiness win on the single-admin model; prereq PREREQ-A ✅; no new infra. P2–P5 follow by capacity |
