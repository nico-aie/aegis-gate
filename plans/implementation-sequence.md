...
| ◑ | **zone-aware-load-balancing** — **P1+P2+P4 shipped** (P1 node self-zone; P2 `pick_with_locality` same-zone preference; **P4** capacity gate `min_local_healthy_pct` spills a half-dead local zone). Only P3 observability remains (dashboard self-zone readout + per-zone summary + served-local metric) | Foundation | M | Locality routing; shares the `LbStrategy::pick` touchpoint |
