...
| [jwt-inspection.md](./jwt-inspection.md) | `jwt_alg_none`, `jwt_x5c_inline`, `jwt_kid_injection`, `jwt_jku_external`, `jwt_time_forged`, `jwt_role_priv` | `Authorization: Bearer` + `Cookie` JWTs — decodes header/payload, flags **attack shapes** (alg:none any-case, inline `x5c`/`jwk`, `kid` traversal/SQLi, external `jku`/`x5u`, forged time claims). **Detection-only — no signature verification.** Structural rules score 80/70; `jwt_role_priv` is an opt-in observe heuristic (20, `flag_privileged_roles`). Added 2026-06-12 (A1–A3). |
