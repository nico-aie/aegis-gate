...
| WAF decision latency p50/p95/p99 | `waf.handle_data_request` span duration |
...
> the audit ring + the copilot snapshot). Add it here once the metrics
> pipeline (Collector `prometheus` receiver → SigNoz) is live.
